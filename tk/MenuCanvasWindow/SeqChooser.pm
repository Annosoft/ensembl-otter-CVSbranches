...
package MenuCanvasWindow::SeqChooser;
...
sub show_LocusWindow{
    ...
    unless($window){
        ...
        print STDERR "creating LocusWindow from ".$self->Client->browser."\n" ;
...
#            warn "updating ace db for " . $SubSeq->name;  
    ...
    my $result = $self->update_display($ace);  
    unless ($result == 0){
...
sub populate_menus {
    ...
    my $browser = $self->Client->browser;
    # Launce xace
    my $xace_launch_command = sub { $self->launch };
    $file->add('command',
    ...
    # Attach xace
    my $xace_attach_command = sub { $self->attach };
    $file->add('command',
...
=head1 NAME - MenuCanvasWindow::SeqChooser
