...
my ($total_found,$total_not_found,$total_external);
...
foreach my $chr (@chr_sorted) {
	...
	my ($havana_c,$external_c,$found,$not_found);
	...
	$support->log("Analysing External genes...\n");
	foreach my $gene (@$genes) {
		my $gene_name = $gene->display_xref->display_id;
        my $gid = $gene->dbID;
 		my $gsi = $gene->stable_id;
		...
		if ( $havana_genes->{$stripped_name} ) {
			my $pid = $havana_genes->{$stripped_name};
